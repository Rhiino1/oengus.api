 ...
 * 
 * @param {Object} data Data from GET request
 * @returns {boolean} True if data is a valid MarathonDonationsStats, otherwise returns false
 */
 ...
 * 
 * @param {Object} data Data from GET request
 * @returns {boolean} True if data is a valid Donation, otherwise returns false
 */
const isDonation = (data) => {
  if (
    ...
    (typeof data.donationComment !== 'string')
  ) {
    if (!data.donationIncentiveLinks || typeof data.donationIncentiveLinks !== 'object' ||
      data.donationIncentiveLinks.find(incentive => {
        return !isIncentive(incentive)
      }) !== undefined) {
